#include "TraceStrike.h"
#include "../../XCFrameInfo.h"
#include "../XCRender/RenderManager.h"
#include "../XCRender/ParticleHelper.h"
#include <GL3/gl3w.h>
XCWavFile TraceStrike::strikeEffect;
bool TraceStrike::isResInit = false;

TraceStrike::TraceStrike(float x, float y, float z)
{
	NowPosition[0] = x;
	NowPosition[1] = y;
	NowPosition[2] = z;
...
void TraceStrike::Render()
{
	if (isInit) {
		...
		renderHelper->Render(glm::vec3(NowPosition[0], NowPosition[1], NowPosition[2]),
			glm::vec4(1.0f, 1.0f, 1.0f, 0.5f), glm::radians(90.0f), glm::vec3(0, 0, 1), glm::vec3(scaleSize[0], scaleSize[1], scaleSize[2]),
...
void TraceStrike::checkCollisionWithBoss(BossObject * pBoss)
{
...
void TraceStrike::checkCollisionWithEnemy(EnemyObject * pEnemy)
{
