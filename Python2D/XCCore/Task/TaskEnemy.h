...
public:
	TaskEnemy(std::string taskUuid, std::string targetTaskUuid, int repeatTime, int intervalFrame ,
		std::string enemyImage, glm::vec2 divideInfo, glm::vec3 scaleInfo,glm::vec2 standbyInfo, glm::vec2 walkInfo,
		glm::vec3 initCoord, float velocity, float acceleration, float angle, float angleAcceleration, int type, float enemyHealth);
