...
void DecisionPointSpecialEffect::SpecialEffectRender(float x, float y, float z)
{
	if (getIsInit()) {
		...
		image->Render(glm::vec3(x, y, z), glm::vec4(1.0f), glm::radians((float)timer.getAccumlateTime()*90.0f), glm::vec3(0, 0, 1), 
			glm::vec3(0.12f * XCFrameInfo::FrameRight,0.12f * XCFrameInfo::FrameTop,1.0f), IRenderHelper::GetSpecificTexture( 1, 1, 1, 1));

		image->Render(glm::vec3(x, y, z), glm::vec4(1.0f), glm::radians((-(float)timer.getAccumlateTime()*90.0f + 90.0f)), glm::vec3(0, 0, 1), 
			glm::vec3(0.12f * XCFrameInfo::FrameRight, 0.12f * XCFrameInfo::FrameTop, 1.0f), IRenderHelper::GetSpecificTexture( 1, 1, 1, 1));
		glDisable(GL_BLEND);
	}
}
